  ...
  public function applies(RouteMatchInterface $route_match) {
    ...
    foreach ($spt_table as $value) {
      ...
      if ($value['status'] == 1) {
		...
		if ($node && !in_array('edit', $path_args)) {  
      $allowed_node_bundles = ['landing_page_thank_you_pages', 'ltp', 'fest_programs', 'fest_programs_list_page', 'fest_speakers', 'fest_speakers', 'streaming_pages', 'speakers_artists_list_page', 'retreat_live_pages', 'retreat_landing_pages']; 
			if(in_array($node->bundle(), $allowed_node_bundles)) 
				$condition = TRUE; 
		}
